...
module MobileCenterClient
  ...
  class ReleaseUploads
    ...
    def complete_async(upload_id, owner_name, app_name, status, custom_headers = nil)
      ...
      # Serialize Request
      request_mapper = MobileCenterClient::Models::ReleaseUploadEndRequest.mapper()
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            ...
            result_mapper = MobileCenterClient::Models::ReleaseUploadEndResponse.mapper()
    ...
    def create_async(owner_name, app_name, custom_headers = nil)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 201
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = MobileCenterClient::Models::ReleaseUploadBeginResponse.mapper()
            result.body = @client.deserialize(result_mapper, parsed_response, 'result.body')
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end

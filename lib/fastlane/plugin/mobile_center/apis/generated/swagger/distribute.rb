...
module swagger
  ...
  class Distribute
    ...
    def post_v01users_by_user_id_devices_register_async(user_id, body, custom_headers = nil)
      ...
      # Serialize Request
      request_mapper = swagger::Models::DeviceInfoRequest.mapper()
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            ...
            result_mapper = swagger::Models::DeviceInfoResponse.mapper()
    ...
    def get_v01devices_register_async(original_url, custom_headers = nil)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = swagger::Models::DeviceConfigurationResponse.mapper()
            result.body = @client.deserialize(result_mapper, parsed_response, 'result.body')
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
